...
complete_kri_human_ephys = joined_ephys_data %>% filter(layer_name %in% c('L2.3', 'L5')) %>% dplyr::select(kri_ephys_features, ) %>% drop_na %>% as.data.frame()
rownames(complete_kri_human_ephys) = joined_ephys_data %>% filter(layer_name %in% c('L2.3', 'L5')) %>% dplyr::select(c(kri_ephys_features, 'cell_id'), ) %>% drop_na %>% pull(cell_id)
...
complete_kri_human_ephys_umap_new %>% ggplot(aes(x = V1, y = V2, color = layer_name, shape = recorder_name)) + geom_point(size = 2)
