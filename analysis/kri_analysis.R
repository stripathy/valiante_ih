...
# layer 2-3
ephys_meta = read_excel(path = demo_file_name, sheet = l23_sheet_name, range = 'B5:H10', col_names = F)
...
ephys_meta = read_excel(path = demo_file_name, sheet = l23_sheet_name, range = 'B19:K25', col_names = F)
...
ephys_meta = read_excel(path = demo_file_name, sheet = l3_sheet_name, range = 'A3:F8', col_names = F)
...
iv_ranges = c('A2:BZ10', 'A2:AK18', 'A2:AA10', 'A2:AC18', 'A2:Q10',
...
kri_ephys_data = lapply(1:length(ephys_sheets), function(sheet_name){
  ...
  rmp_data$layer_name = cell_layer_vec[sheet_name]
  rmp_data$cell_type = cell_type_vec[sheet_name]
  print(rmp_data)
  return(rmp_data)
}) %>% bind_rows()
...
joined_ephys_data = left_join(patient_cell_meta %>% select(-layer_name), kri_ephys_data %>% distinct(cell_id, .keep_all = T)) %>% 
